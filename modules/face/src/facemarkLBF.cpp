...
namespace cv
{
    ...
    */
    ...
    void FacemarkLBFImpl::trainingImpl(String imageList, String groundTruth){
        ...
        prepareTrainingData(images, facePoints, shapes, cropped, boxes, cc);
    ...
    void FacemarkLBFImpl::prepareTrainingData(std::vector<String> images, std::vector<std::vector<Point2f> > & facePoints, std::vector<Mat> & cropped, std::vector<Mat> & shapes, std::vector<BBox> &boxes, CascadeClassifier cc){
        ...
        for(int i=0; i<(int)images.size();i++){
            ...
            if(box.x != -1){
                ...
                // cout<<boxes.back().x<<" "<<boxes.back().y<<" "<< boxes.back().width<<" "<< boxes.back().height<<endl;
                // drawPoints(img, facePoints[i]);
                // rectangle(img, Rect(boxes.back().x,boxes.back().y, boxes.back().width, boxes.back().height), Scalar(255,0,0));
                // imshow("mark", img);
                // imshow("crop", crop);
                // waitKey(0);
            }
    ...
    FacemarkLBFImpl::BBox::BBox(double _x, double _y, double _w, double _h) {
        this->x = _x; this->y = _y;
        this->width = _w; this->height = _h;
        this->x_center = _x + _w / 2.;
        this->y_center = _y + _h / 2.;
        this->x_scale = _w / 2.;
        this->y_scale = _h / 2.;
    }
